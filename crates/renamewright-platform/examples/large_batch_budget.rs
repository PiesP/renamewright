...
use renamewright_core::{
    MAX_PLAN_TRACE_BYTES, MAX_RULES, PlanId, RenamePlan, RenameRule, RulePipeline, SequenceOrder,
    SequencePlacement, SequenceScope, TargetPolicy, build_plan_with_rule_pipeline,
};
...
const SEQUENCE_PLAN_BUDGET: Duration = Duration::from_secs(2);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let sequence_heavy = measure_plan(
        PlanId::new(3),
        &snapshots,
        (0..8)
            .map(|index| {
    ...
    enforce_duration(
        "sequence-heavy planning",
        sequence_heavy.elapsed,
        SEQUENCE_PLAN_BUDGET,
    )?;
    ...
    println!(
        "{{\"sourceCount\":{SOURCE_COUNT},\"admissionMs\":{},\"representativePlanMs\":{},\"expandingPlanMs\":{},\"sequencePlanMs\":{},\"representativeRetainedBytes\":{},\"expandingRetainedBytes\":{},\"retainedTraceBytes\":{},\"traceTruncatedRows\":{},\"peakRssBytes\":{},\"peakRssBudgetBytes\":{PEAK_RSS_BUDGET_BYTES}}}",
        admission_elapsed.as_millis(),
        representative.elapsed.as_millis(),
        expanding.elapsed.as_millis(),
        sequence_heavy.elapsed.as_millis(),
