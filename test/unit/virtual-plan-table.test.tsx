...
test('windows a large plan and filters without rendering every row', async () => {
  ...
  expect(screen.getAllByText(/Duplicate destination/u).length).toBeGreaterThan(0);
  expect(screen.getAllByText(/Destination already exists/u).length).toBeGreaterThan(0);
  expect(screen.getAllByText(/Source changed since admission/u).length).toBeGreaterThan(0);
  expect(screen.getAllByText(/Source directory could not be validated/u).length).toBeGreaterThan(0);
});

test('combines status, diagnostic, and affected-source filters without expanding the window', async () => {
  const user = userEvent.setup();
  const rows = makeRows(10_000);
  const target = rows.at(-1);
  if (!target) {
    throw new Error('Filter fixture is incomplete.');
  }
  target.originalName = 'quarterly-report.pdf';

  render(() => <VirtualPlanTable rows={rows} overrides={[]} onOverride={() => {}} />);

  await user.selectOptions(screen.getByRole('combobox', { name: 'Diagnostic' }), 'staleSource');
  await user.type(screen.getByRole('searchbox', { name: 'Affected source' }), 'quarterly');
  await user.click(screen.getByRole('button', { name: 'Blocked 1' }));
  ...
  await user.clear(screen.getByRole('searchbox', { name: 'Affected source' }));
  await user.type(screen.getByRole('searchbox', { name: 'Affected source' }), 'missing');
  ...
  if (!emptyStateClear) {
    throw new Error('Filter reset action is unavailable.');
  }
  await user.click(emptyStateClear);
  expect(screen.getByText('Showing 10000 of 10000')).toBeInTheDocument();
  expect(screen.getByRole('searchbox', { name: 'Affected source' })).toHaveValue('');
  expect(screen.getByRole('combobox', { name: 'Diagnostic' })).toHaveValue('all');
  expect(screen.getAllByRole('row').length).toBeLessThan(30);
});
