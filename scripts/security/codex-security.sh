...
Environment overrides:
  ...
  CODEX_SECURITY_FAIL_ON_SEVERITY  Hook severity threshold (default: high)
...
args=(
  ...
  --knowledge-base "$repo_root/.github/codex-security/threat-model.md"
  --knowledge-base "$repo_root/.github/SECURITY.md"
...
case "$mode" in
  ...
  hook)
    args+=(
      --working-tree
      --base HEAD
      --auth "${CODEX_SECURITY_AUTH:-chatgpt}"
      --fail-on-severity "${CODEX_SECURITY_FAIL_ON_SEVERITY:-high}"
    )
    ;;
  working-tree)
